...
@section('scripts')
    ...
    <script>
        $("#cover_box").dropzone({
            ...
            url: "{{route('style.category.cover.upload')}}",
            addRemoveLinks: true,
            maxFiles: 1,
        ...
        $('#btn-commit').click(function(){
            $.ajax({
                type : 'post',
                url : "{{route('merchant.style.category.store')}}",
                ...
                success : function(data,textStatus,jqXHR){
                    if(data.error == 0){
                        alert('添加成功');
                        window.location.href = "{{route('merchant.style.category.index')}}";
                    }else{
                        alert(data.message);
                    }
                }
